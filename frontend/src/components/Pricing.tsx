...
export default function Pricing() {
  ...
  return (
    <section id="pricing" className="py-24 bg-[#F2F6F5] relative overflow-hidden">
      {/* Visual background accents */}
      <div className="absolute top-10 left-10 w-96 h-96 bg-[#86ABA5]/10 rounded-full blur-3xl pointer-events-none" />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="text-center max-w-3xl mx-auto mb-16" id="pricing-title-block">
          ...
          <h2 className="text-3xl sm:text-4xl font-display font-extrabold text-[#1E302D] tracking-tight mb-4">
          ...
          <div className="mt-8 flex justify-center items-center gap-3" id="pricing-toggle-box">
            <span className={`text-xs sm:text-sm font-semibold transition-colors ${!isAnnual ? "text-[#FF5A36]" : "text-slate-400"}`}>
              月払い
            </span>
            <button
              onClick={() => setIsAnnual(!isAnnual)}
              className="w-12 h-6 rounded-full bg-[#1E302D] p-1 transition-all flex items-center relative duration-300"
              id="billing-toggle"
            >
              <div className={`w-4 h-4 rounded-full bg-[#86ABA5] absolute transition-all duration-300 ${isAnnual ? "right-1" : "left-1"}`} />
            </button>
            <span className={`text-xs sm:text-sm font-semibold transition-colors flex items-center gap-1.5 ${isAnnual ? "text-[#FF5A36]" : "text-slate-400"}`}>
              年払い
              <span className="text-[10px] font-extrabold bg-[#FF5A36] text-white px-1.5 py-0.5 rounded-full uppercase leading-none">
                -20%
              </span>
            </span>
          </div>
            ...
            return (
              <motion.div
                ...
                className={`relative rounded-3xl p-8 flex flex-col justify-between border transition-all duration-300 text-left ${
                  isPopular
                    ? "bg-[#1E302D] border-[#FF5A36] shadow-2xl scale-102 lg:scale-105 z-10 text-white"
                    : "bg-[#86ABA5]/70 border-white/40 shadow-xl text-slate-900"
                }`}
                id={`pricing-card-${idx}`}
              >
                {/* Popular badge */}
                {isPopular && (
                  <span className="absolute -top-3.5 left-1/2 -translate-x-1/2 bg-[#FF5A36] text-white px-4 py-1 rounded-full text-[10px] font-extrabold tracking-widest uppercase shadow-md shadow-[#FF5A36]/20 flex items-center gap-1">
                ...
                <div>
                  {/* Name and desc */}
                  <div className="mb-6">
                    <h3 className={`text-xl sm:text-2xl font-display font-extrabold ${isPopular ? "text-[#86ABA5]" : "text-[#1E302D]"}`}>
                      {plan.name}
                    </h3>
                  ...
                  {/* Pricing line */}
                  <div className="mb-8 border-y border-[#1E302D]/10 py-5 flex flex-col">
                    <div className="flex items-baseline gap-1">
                      <span className={`text-3xl sm:text-4xl font-display font-black tracking-tight ${isPopular ? "text-[#FF5A36]" : "text-[#1E302D]"}`}>
                        {displayedPrice}
                      </span>
                    ...
                    {isAnnual && plan.price !== "お問い合わせ" && plan.price !== "Liên hệ" && (
                      <span className="text-[10px] font-mono text-[#FF5A36] mt-1 font-bold">
                  ...
                  <div className="space-y-4 mb-8">
                    {plan.features.map((feat, fIdx) => (
                      <div key={fIdx} className="flex items-start gap-3">
                        <Check className={`h-4 w-4 mt-0.5 shrink-0 ${isPopular ? "text-[#86ABA5]" : "text-slate-900"}`} />
                ...
                {/* Pricing button */}
                <div className="mt-auto pt-6 border-t border-[#1E302D]/10 flex flex-col gap-3">
                  <a
                    ...
                    className={`w-full py-3.5 px-4 rounded-xl font-bold text-xs sm:text-sm text-center transition-all duration-300 flex items-center justify-center gap-2 ${
                      isPopular
                        ...
                        : "bg-[#1E302D] hover:bg-slate-800 text-white shadow-sm hover:-translate-y-0.5"
                    }`}
                  >
                    {plan.buttonText}
                  ...
                  <div className="flex justify-between items-center px-1 text-[10px] font-mono font-bold">
                    <a href="#contact" className="hover:text-[#FF5A36] underline transition-colors">詳細を比較</a>
                    {plan.name !== "エンタープライズプラン" ? (
                      <a href="#contact" className="hover:text-[#FF5A36] flex items-center gap-0.5 transition-colors">
                        カスタムリクエスト
                      </a>
                    ) : (
        ...
        <div className="p-6 rounded-2xl bg-[#86ABA5]/10 border border-[#86ABA5]/25 max-w-3xl mx-auto flex gap-3 text-left">
